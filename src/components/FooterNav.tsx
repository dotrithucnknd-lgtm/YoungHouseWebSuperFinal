...
const FooterNav = () => {
  ...
  const renderItem = (item: NavItem, index: number) => {
    ...
    return item.link ? (
      ...
      >
        ...
        <span
          className={`text-[11px] leading-none mt-1 ${isActive ? "text-primary-600 font-medium" : ""
            }`}
        >
          {item.name}
        </span>
  ...
  return (
    <div
      ref={containerRef}
      className="FooterNav block md:!hidden h-16 p-2 bg-white dark:bg-neutral-850 fixed top-auto bottom-0 inset-x-0 z-50 border-t border-neutral-300 dark:border-neutral-700 

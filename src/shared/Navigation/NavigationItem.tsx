...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
  ...
  const renderMainItem = (item: NavItemType) => {
    const isActive = item.href === "/" ? locationPathName === "/" : locationPathName?.startsWith(item.href as string);
    return (
      <Link
        rel="noopener noreferrer"
        className={`inline-flex items-center text-sm xl:text-base font-normal text-neutral-700 dark:text-neutral-300 py-2 px-4 xl:px-5 rounded-full hover:text-neutral-900 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:hover:text-neutral-200 ${
          isActive
            ? "relative text-primary-700 dark:text-primary-500 after:content-[''] after:absolute after:left-4 after:right-4 after:-bottom-0.5 after:h-0.5 after:bg-primary-500 after:rounded-full"
            : ""
        }`}
        href={item.href || "/"}

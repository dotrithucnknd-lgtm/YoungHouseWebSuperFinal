...
import { uploadImage, uploadMultipleImages, DatabaseNearbyPlace, addRoomUniversities, fetchRoomVideoReviewsByRoomId } from "@/lib/supabaseServices";
...
export default function AdminEditRoomPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (error || !room) { router.push("/admin/rooms"); return; }

      setOwnerId(room.owner_id || null);
      setFormData({
      ...
      if (room.owner_id) {
        const { data: profile } = await supabase
          .from("profiles").select("phone").eq("id", room.owner_id).single();
      ...
      const { data: roomAmenities } = await supabase
        .from("room_amenities").select("amenity_id").eq("room_id", roomId);
      if (roomAmenities) setSelectedAmenities(new Set(roomAmenities.map((ra: any) => ra.amenity_id)));

      // Universities
      const { data: unis } = await supabase
        .from("room_universities").select("university_id").eq("room_id", roomId);
      ...
      // Video reviews
      const { rows: vidRows } = await fetchRoomVideoReviewsByRoomId(roomId);
      if (vidRows && vidRows.length > 0) {
        setVideoReviews(vidRows.map((v: any, i: number) => ({
          source_url: v.source_url, display_title: v.display_title || "", sort_order: i
        })));
      } else {
        setVideoReviews([{ source_url: '', display_title: '', sort_order: 0 }]);
      }
    } catch (err) {
  ...
  const handleDeleteExistingImage = (imageId: string) => { setImagesToDelete(prev => [...prev, imageId]); setExistingImages(prev => prev.filter(img => img.id !== imageId)); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title.trim()) { alert('Vui lòng nhập tên'); return; }
    ...
    try {
      ...
      // Update owner phone
      if (ownerId && formData.phone.trim()) {
        await supabase.from('profiles').update({ phone: formData.phone.trim() }).eq('id', ownerId);
      }
      ...
      if (allNewImages.length > 0) {
        await supabase.from('room_images').insert(allNewImages.map(url => ({ room_id: roomId, image_url: url })));
      }

      // Update amenities
      await supabase.from('room_amenities').delete().eq('room_id', roomId);
      if (selectedAmenities.size > 0) {
        await supabase.from('room_amenities').insert(Array.from(selectedAmenities).map(amenityId => ({ room_id: roomId, amenity_id: amenityId })));
      }

      // Update universities
      await supabase.from('room_universities').delete().eq('room_id', roomId);
      if (selectedUniversities.length > 0) {
        await addRoomUniversities(roomId, selectedUniversities);
      }

      // Update nearby places
      await supabase.from('nearby_places').delete().eq('room_id', roomId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link href={`/admin/rooms/${roomId}`} className="p-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg transition-colors">
          <ArrowLeftIcon className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-neutral-900 dark:text-white">Chỉnh sửa phòng trọ</h1>
          <p className="text-sm text-neutral-500 mt-1">Cập nhật thông tin phòng trọ</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">SĐT liên hệ chủ trọ</label>
              <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange}
                className="w-full px-4 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              <p className="text-xs text-neutral-500 mt-1">Cập nhật sẽ áp dụng cho tất cả phòng của chủ trọ này</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            <button type="submit" disabled={loading}
              ...
              {loading ? (
                <><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> Đang lưu...</>
              ) : 'Lưu thay đổi'}
            </button>
